...
//////
static size_t mg_mqtt_next_topic(struct mg_mqtt_message *msg, struct mg_str *topic, uint8_t *qos,
                                 size_t pos) {
...
size_t mg_mqtt_next_sub(struct mg_mqtt_message *msg, struct mg_str *topic, uint8_t *qos,
                        size_t pos) {
   uint8_t tmp;
      ...
      switch (mm->cmd) {
         ...
         case MQTT_CMD_SUBSCRIBE: {
            ...
            while ( ( (pos = mg_mqtt_next_sub(mm, &topic, &qos, pos) ) > 0) ) {
               ...
               Log(LOG_DEBUG, "mqtt.req", "SUB %p [%.*s]", c->fd, (int) sub->topic.len,
                  sub->topic.buf);
