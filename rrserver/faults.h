...
// XXX: We need set the highest bit to 1 to indicate that fault is fatal and
// should halt TX
// A higher (numeric) error will replace a lower numbered one in the fault code
// register...
// XXX: Sort these by order of severity
#define	FAULT_NONE 0x00           // No faults
#define	FAULT_STUCK_RELAY 0x01    // Power On Self Test determined a relay
                                   // is stuck
...
#define	FAULT_FINAL_LOW_CURRENT 0x06 // Final supply current much below
                                     // expected
...
#define	FAULT_WARMING_UP 0x0b     // PTT attempted while warming up (will
                                   // be cleared by warmup routine)
...
extern int fault_priority(uint32_t fault);
extern const char *fault_get_type_str(uint32_t fault);
extern bool fault_is_fatal(uint32_t fault);
extern uint32_t set_fault(uint32_t fault);
extern bool check_faults(void);
