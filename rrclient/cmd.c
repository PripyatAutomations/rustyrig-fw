...
bool parse_chat_input_real(const char *msg) {
   ...
   if (msg[0] == '/') {
      ...
      /*
       * Work on a writable copy since we're going to replace
       * whitespace with NUL terminators.
       */
      ...
      /*
       * Keep this reasonably sized. cmd_argv is only used for
      ...
      /*
       * max_args is the number of arguments after argv[0].
       * Zero means one argument containing the remainder.
       */
      ...
      while (*p && cmd_argc < (int)(sizeof(cmd_argv) / sizeof(cmd_argv[0]))) {
         ...
         /*
          * If this is the last allowed argument, leave the
          * remainder of the string intact.
          */
      ...
      Log(LOG_CRIT, "chat.cmd",
         "command=%s argc=%d max_args=%d",
         cmd_argv[0], cmd_argc, max_args);
