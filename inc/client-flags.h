...
#else
static inline bool has_flag(struct rr_user *cptr, u_int32_t user_flag) {
   if (cptr) {
...
static inline void set_flag(struct rr_user *cptr, u_int32_t flag) {
   if (cptr) {
      cptr->user_flags |= flag;
   }
}

static inline void clear_flag(struct rr_user *cptr, u_int32_t flag) {
   if (cptr) {
      cptr->user_flags &= ~flag;
   }
}
