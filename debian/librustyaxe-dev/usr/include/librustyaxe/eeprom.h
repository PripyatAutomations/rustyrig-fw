...
struct eeprom_layout {
   ...
   ee_data_type type;                    // this is generated by buildconf.pl as
                                         // build/$profile/eeprom_types.h
};

struct eeprom_channel_groups {
   // Channel group names
   int id;
   char name[13];
};
...
struct eeprom_channel_header {
   // Header data for the channel memory
...
struct eeprom_channel_slot {
   // An indvidual channel slot
   int group;                            // Channel Group or -1 if none
   char name[29];                        // Name string
   float freq;                           // Frequency in khz
   char mode[5];                         // Modulation mode
//    enum mod_mode  mode;    // Modulation Mode (stored as 4 byte string in
// eeprom)
   int bw;                               // bw in hz
   int split_dir;                        // Split direction: -1 for negative, 0
                                         // or 1 for normal
   float tx_offset;                      // Transmit offset in Khz (for
                                         // repeaters)
...
extern bool eeprom_corrupted;                    // EEPROM is corrupted; prompt
                                                 // before write out
extern time_t eeprom_saved;              // When was EEPROM last written out?
extern time_t eeprom_changed;                    // When was a setting last
                                                 // changed that needs written
                                                 // to
                                                 // EEPROM?
