...
bool ws_handle_media_msg(struct mg_connection *c, dict *d) {
   ...
   if (media_cmd && strcasecmp(media_cmd, "capab") == 0) {
      ...
      if (codec_msg) {
         ...
         Log(LOG_CRIT, "ws.media",
            ">> No codecs enabled locally or failed to generate codec message <<");
      }
      free( (void *)preferred );
   } else if (media_cmd && strcasecmp(media_cmd, "isupport") == 0) {
      char *media_preferred = dict_get(d, "media.preferred", NULL);
      if (media_codecs) {
         Log(LOG_DEBUG, "ws.media", "Server negotiated codecs: %s, preferred: %s", media_codecs,
            media_preferred);
         ...
         // Copy the *first* codec of the negotiated set, as it's our most
         // preferred.
         memcpy(first_codec, media_codecs, 4);
// XXX: need to move this to the GTK code and have it as a callback
//         populate_codec_combo(GTK_COMBO_BOX_TEXT(tx_codec_combo),
// media_codecs, (media_preferred ? media_preferred : "pc16"));
//         populate_codec_combo(GTK_COMBO_BOX_TEXT(rx_codec_combo),
// media_codecs, (media_preferred ? media_preferred : "pc16"));
//         ws_select_codec(c, first_codec, false);
      ...
      Log(LOG_DEBUG, "ws.media," ">> Unknown media.cmd: %s, .payload: %s", media_cmd,
         media_payload);
      goto cleanup;
   }
cleanup:
