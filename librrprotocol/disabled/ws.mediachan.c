//
// rrserver/ws.mediachan.c: Handle listing, (un)subscribing, and creating media
// channels
//
...
bool ws_list_channels(http_client_t *cptr) {
   ...
   while (lp) {
      ...
      const char *jp = dict2json_mkstr(VAL_STR, "media.cmd", "list-reply", VAL_STR, "media.from",
         cptr->chatname, VAL_STR, "media.chan-uuid", lp->chan_uuid, VAL_STR, "media.chan-descr",
         escaped_descr ? escaped_descr : "*** No description ***", VAL_LONG, "media.ts", now);
      fprintf(stderr, "jp: %s\n", jp);
...
bool ws_unsubscribe_channel(http_client_t *cptr, int chan_id) {
   ...
   mediachan_sub_t *prev_sp = NULL;      // pointer to the previous entry, if
                                         // present
